# VERSION: 2026-04-04 09:30 PM
# STATUS: Restored Phase 2 Independent Logic
# ----------------------------------------------------------------
...
day_date = st.date_input("Workout Date", value=date.today())
...
for ex in gym_ex:
    with st.expander(ex):
        # Unique key for sets count
        sets_count = st.number_input(f"Sets", 1, 10, 4, key=f"sets_{week}_{day}_{ex}")
        
        # Simple copy set 1 to others logic
        if st.button("📋 Copy Set 1 to All", key=f"copy_{ex}"):
        ...
        for s in range(1, sets_count + 1):
            ...
            # Initial Data Load into Session State
            ...
            # THE INDEPENDENT SAVE BUTTON
            if c4.button("💾", key=f"btn_{w_key}"):
                df_curr = load_data()
                # Remove ONLY this specific set/exercise
