# VERSION: 2.05
# STATUS: Phase 2 - Added "Bodyweight" option to Gym Exercises
# ----------------------------------------------------------------
...
def on_data_change(week, day, date_val, exercise, set_num, weight_key, reps_key):
    ...
    # Convert "Bodyweight" string back to 0.0 for storage
...
# ---------------- GYM EXERCISES ----------------
# Options: "Bodyweight" followed by 2.5lb increments
    ...
    with st.expander(ex):
        ...
        for s in range(1, sets_count + 1):
            ...
            if s_weight == 0.0:
                ...
                except: w_idx = 2 # Default to 5.0 if error
            
            c1, c2, c3 = st.columns([1, 2, 2])
            with c1: st.write(f"**{s}** {'✅' if not saved.empty else ''}")
            with c2:
                st.selectbox("W", weight_options, index=w_idx, 
