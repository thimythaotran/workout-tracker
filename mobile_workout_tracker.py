...
# Password — set via env var WORKOUT_PASSWORD, fallback to “1” for local use
...
“”“Load workout CSV. Returns empty DataFrame with correct columns on any failure.”””
...
st.warning(f”⚠️ Could not load data: {e}. Starting fresh.”)
return pd.DataFrame(columns=REQUIRED_COLUMNS)

def save_data(df: pd.DataFrame):
“”“Save dataframe to CSV safely using a temp file to avoid corruption.”””
tmp = DATA_FILE + “.tmp”
try:
...
“”“Clear the cached dataframe so next read pulls fresh data.”””
if “cached_df” in st.session_state:
del st.session_state[“cached_df”]

def get_data() -> pd.DataFrame:
“”“Session-cached data load — avoids repeated file I/O within a single render cycle.”””
...
“”“Insert or update a single exercise row, then invalidate cache.”””
df = load_data()
mask = (
(df[“Week”] == week) &
(df[“Day”] == day) &
...
“”“Write a DAY MARKER row if one doesn’t exist yet for this week/day.”””
df = get_data()
exists = not df[
...
w1_key = f”weight_{week}*{day}*{exercise}*1”
r1_key = f”reps*{week}*{day}*{exercise}*1”
...
st.session_state[f”weight*{week}*{day}*{exercise}*{s}”] = raw_w
st.session_state[f”reps*{week}*{day}*{exercise}_{s}”]   = val_r
...
def show_timer(key_suffix: str):
“””
Display a rest timer. Uses a JS-driven countdown approach so it only triggers
a Streamlit rerun every ~1 s via st.fragment (if available) rather than
re-rendering the full app. Falls back to a lightweight placeholder update.
“””
...
```
    # Color transitions: green → yellow → red as rest gets longer
    if elapsed < 60:
        color = "#28a745"   # green  — still fresh
    elif elapsed < 90:
        color = "#ffc107"   # amber  — getting long
    else:
        color = "#FF0000"   # red    — way too long

    timer_html = f"""
        <div style="display:flex; align-items:baseline; gap:10px;">
            <span style="color:{color}; font-size:55px; font-weight:bold; font-family:monospace;">{elapsed}s</span>
...
# Auto-write day marker when can_edit and date is set
...
with c_sets:
sets_count = st.number_input(f”Sets”, 1, 10, 4, key=f”sets_{week}*{day}*{ex}”, disabled=not can_edit)
with c_copy:
st.write(””)
if st.button(“👯 Copy Set 1 to All”, key=f”copy_{week}*{day}*{ex}”, disabled=not can_edit, use_container_width=True):
...
dur_key  = f”abs_dur_{week}*{day}*{ex}*{set_num}”
reps_key = f”abs_reps*{week}*{day}*{ex}*{set_num}”
...
with c1: st.selectbox(“Secs”, list(range(0, 125, 5)), index=int(s_dur / 5), key=dur_key,  disabled=not can_edit)
with c2: st.selectbox(“Reps”, list(range(0, 51)),     index=s_reps,          key=reps_key, disabled=not can_edit)
with c3:
if st.button(“💾”, key=f”btn*{dur_key}”, disabled=not can_edit, use_container_width=True):
...
# Reload fresh after any potential saves
...
# Only trigger a rerun for the timer if it’s actually running AND

# at least 1 second has passed since the last tick — avoids hammering

# Streamlit when nothing else is happening.

if st.session_state[“timer_running”]:
now = time.time()
if now - st.session_state.get(“timer_last_tick”, 0) >= 1.0:
st.session_state[“timer_last_tick”] = now
time.sleep(max(0, 1.0 - (now % 1)))  # sleep to the next whole second
st.rerun()