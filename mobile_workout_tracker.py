...
def on_weight_change(week, day, date_val, exercise, set_num, key):
    ...
    df = load_data()
    # Remove old version of this set to prevent duplicates
...
# --- THE DATA RECOVERY FILTER ---
# Filter by Week and Day. This will find your data even if the date is "wrong" in the file.
    ...
    with st.expander("💪 Abs Section", expanded=True):
        for set_num in [1, 2]:
            ...
            for ex in abs_ex:
                ...
                # Tight layout: 4 parts dropdown, 1 part button
                c1, c2 = st.columns([4, 1]) 
                with c1:
                    st.selectbox(f"{ex} (sec)", list(range(0, 125, 5)), index=6, key=dur_key, disabled=not can_edit, label_visibility="visible")
                with c2:
                    # Removed spacers. The button now sits directly next to the dropdown
